  ...
  tables.forEach((table) => {
    ...
    studentsToRender.forEach((student) => {
      ...
      if (courses && Array.isArray(courses)) {
        const found = courses.find(c => c._id === student.course);
...
async function editStudent(id) {
  ...
  try {
    ...
    // Debug: Log the student data to see the date format
    console.log("Student data for edit:", student);
    console.log("Enrollment date raw:", student.enrollmentDate);
    ...
    if (student.enrollmentDate) {
      try {
        // Try to parse the date and format it for input
        const date = new Date(student.enrollmentDate);
        if (!isNaN(date.getTime())) {
          formattedDate = date.toISOString().split('T')[0];
        } else {
          console.warn("Invalid date format:", student.enrollmentDate);
        }
      } catch (error) {
...
async function loadReportsData() {
  showLoading();
  try {
    // Fetch students and courses if not already loaded
    const [studentsData, coursesData] = await Promise.all([
      fetch(`${API_BASE_URL}/api/students`).then((res) => res.json()),
      fetch(`${API_BASE_URL}/api/courses`).then((res) => res.json()),
    ]);
    renderReportStudentsTable(studentsData);
    renderReportCoursesTable(coursesData);
  } catch (error) {
    console.error("Error loading reports data:", error);
    showNotification("Error loading reports data", "error");
    renderReportStudentsTable([]);
    renderReportCoursesTable([]);
  } finally {
    hideLoading();
  }
}

function renderReportStudentsTable(studentsToRender) {
  const table = document.getElementById("reportStudentsTableBody");
  table.innerHTML = "";
...
function renderReportCoursesTable(coursesToRender) {
  const table = document.getElementById("reportCoursesTableBody");
  table.innerHTML = "";
  ...
  coursesToRender.forEach((course) => {
    ...
    row.innerHTML = `
      ...
      <td>${escapeHtml(course.description)}</td>
      <td>${course.duration}</td>
      <td><span class="status-badge status-${course.status}">${capitalizeFirstLetter(course.status)}</span></td>
    `;
    table.appendChild(row);
  });
} 